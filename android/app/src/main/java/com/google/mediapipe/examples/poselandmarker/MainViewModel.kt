 ...
 *  ViewModel: Ensure no data loss when rotating the screen
 */
class MainViewModel : ViewModel() {

    // Using a complete human posture detector
    private var _model = PoseLandmarkerHelper.MODEL_POSE_LANDMARKER_FULL
    // Computer CPU
    private var _delegate: Int = PoseLandmarkerHelper.DELEGATE_CPU
    // To control the model's confidence threshold for pose detection
    private var _minPoseDetectionConfidence: Float =
        PoseLandmarkerHelper.DEFAULT_POSE_DETECTION_CONFIDENCE
    // Controlling the stability of attitude tracking
    private var _minPoseTrackingConfidence: Float = PoseLandmarkerHelper
        .DEFAULT_POSE_TRACKING_CONFIDENCE
    // Threshold for determining whether a detected person is present or not
